...
class InsertVideoUseCaseTest {
    ...
    @Test
    fun save_vide_to_favorite_test() = runBlocking {

        val useCase = InsertVideoUseCase(repository = videoRepository)

        useCase.saveVideoToFavorite(video)

        Mockito.verify(videoRepository).insertVideo(video)
    }
}